...
class CNN(torch.nn.Module):
    def __init__(self):
        ...
        # batch_size x 8 x 16 x 16
        self.conv3 = torch.nn.Conv2d(8, 16, kernel_size=3, stride=1, padding=1)
        # batch_size x 16 x 16 x 16
        # pool
        # batch_size x 16 x 8 x 8
        self.conv4 = torch.nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1)
        self.norm2 = torch.nn.BatchNorm2d(32)
        # batch_size x 32 x 8 x 8
        self.norm4 = torch.nn.BatchNorm1d(32 * 8 * 8)
        self.fc1 = torch.nn.Linear(32 * 8 * 8, 30 * 30)
        self.norm5 = torch.nn.BatchNorm1d(30 * 30)
        self.fc2 = torch.nn.Linear(30 * 30, 15 * 15)
...
model = CNN()
model.load_state_dict(torch.load("upd", map_location='cpu'))
model.eval()
